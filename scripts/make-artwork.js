...
const C = {
  stroke: '#e7edef',
  edge: 'rgba(231,237,239,0.22)',
  faint: 'rgba(231,237,239,0.12)',
  signal: '#3f9cb4',
  brass: '#d9a441',
};
...
function svg({ w, h, title, code, scale, body }) {
  ...
  <defs>
    <linearGradient id="ground" x1="0" y1="0" x2="0.4" y2="1">
      <stop offset="0" stop-color="#0c1f2b"/><stop offset="0.55" stop-color="#081722"/><stop offset="1" stop-color="#050e15"/>
    </linearGradient>
    ...
    <pattern id="grid-fine" width="8" height="8" patternUnits="userSpaceOnUse">
      <path d="M8 0H0V8" fill="none" stroke="rgba(231,237,239,0.05)" stroke-width="0.6"/>
    </pattern>
    <linearGradient id="wash" x1="0" y1="0" x2="0" y2="1">
      <stop offset="0" stop-color="${C.signal}" stop-opacity="0.42"/><stop offset="1" stop-color="${C.signal}" stop-opacity="0.04"/>
    </linearGradient>
  </defs>
  <rect width="${w}" height="${h}" fill="url(#ground)"/>
  <rect width="${w}" height="${h}" fill="url(#grid-fine)"/>
  <rect width="${w}" height="${h}" fill="url(#grid)"/>
  <rect x="16" y="16" width="${w - 32}" height="${h - 32}" fill="none" stroke="${C.edge}" stroke-width="1"/>
  <line x1="16" y1="${h - 62}" x2="${w - 16}" y2="${h - 62}" stroke="${C.edge}" stroke-width="1"/>
  <text x="34" y="${h - 30}" fill="${C.stroke}" font-family="'IBM Plex Mono',monospace" font-size="16" letter-spacing="2.5" opacity="0.88">${title}</text>
  <text x="${w - 34}" y="${h - 30}" text-anchor="end" fill="${C.brass}" font-family="'IBM Plex Mono',monospace" font-size="13" letter-spacing="2" opacity="0.9">MERIDIAN</text>
  <text x="34" y="46" fill="${C.signal}" font-family="'IBM Plex Mono',monospace" font-size="13" letter-spacing="3">${code}</text>
  <text x="${w - 34}" y="46" text-anchor="end" fill="${C.edge}" font-family="'IBM Plex Mono',monospace" font-size="12" letter-spacing="2">${scale}</text>
${body}
</svg>
`;
}
...
const sheets = [
  ...
  /* Services -------------------------------------------------------------- */
  { file: 'svc-structural.svg', motif: 'tower', title: 'STRUCTURAL ENGINEERING · FRAME', code: 'S-01', scale: 'SCALE 1:200', size: LAND },
  { file: 'svc-civil.svg', motif: 'viaduct', title: 'CIVIL AND INFRASTRUCTURE · DECK', code: 'S-02', scale: 'SCALE 1:500', size: LAND },
  { file: 'svc-mechanical.svg', motif: 'plant', title: 'MECHANICAL AND PROCESS · RACK', code: 'S-03', scale: 'SCALE 1:150', size: LAND },
  { file: 'svc-digital.svg', motif: 'model', title: 'DIGITAL ENGINEERING · FEDERATED', code: 'S-04', scale: 'SCALE 1:200', size: LAND },
  { file: 'svc-steel.svg', motif: 'canopy', title: 'STEEL FABRICATION · ERECTION', code: 'S-05', scale: 'SCALE 1:250', size: LAND },
  { file: 'svc-groundworks.svg', motif: 'piles', title: 'GROUNDWORKS · PILE SECTION', code: 'S-06', scale: 'SCALE 1:100', size: LAND },
  { file: 'svc-plant.svg', motif: 'refinery', title: 'PLANT CONSTRUCTION · GA', code: 'S-07', scale: 'SCALE 1:400', size: LAND },
  { file: 'svc-commissioning.svg', motif: 'facade', title: 'COMMISSIONING · WITNESS GRID', code: 'S-08', scale: 'SCALE 1:100', size: LAND },

  /* Projects -------------------------------------------------------------- */
  { file: 'proj-helix-tower.svg', motif: 'tower', title: 'HELIX TOWER · ELEVATION', code: 'P-01', scale: 'SCALE 1:500', size: LAND },
  { file: 'proj-north-crossing.svg', motif: 'bridge', title: 'NORTH CROSSING · ELEVATION', code: 'P-02', scale: 'SCALE 1:1000', size: LAND },
  { file: 'proj-atlas-plant.svg', motif: 'plant', title: 'ATLAS PROCESS PLANT · GA', code: 'P-03', scale: 'SCALE 1:600', size: LAND },
  { file: 'proj-rhone-transit.svg', motif: 'canopy', title: 'RHONE TRANSIT HUB · SECTION', code: 'P-04', scale: 'SCALE 1:400', size: LAND },
  { file: 'proj-kade-viaduct.svg', motif: 'viaduct', title: 'KADE VIADUCT · ELEVATION', code: 'P-05', scale: 'SCALE 1:800', size: LAND },
  { file: 'proj-haven-terminal.svg', motif: 'portal', title: 'HAVEN TERMINAL · SECTION', code: 'P-06', scale: 'SCALE 1:300', size: LAND },
  { file: 'proj-westgate-foundations.svg', motif: 'piles', title: 'WESTGATE · FOUNDATION SECTION', code: 'P-07', scale: 'SCALE 1:150', size: LAND },
  { file: 'proj-delta-refinery.svg', motif: 'refinery', title: 'DELTA REFINERY · GA', code: 'P-08', scale: 'SCALE 1:750', size: LAND },
  { file: 'proj-brede-warehouse.svg', motif: 'portal', title: 'BREDE WAREHOUSE · PORTAL FRAME', code: 'P-09', scale: 'SCALE 1:250', size: LAND },
  { file: 'proj-zuidas-retrofit.svg', motif: 'facade', title: 'ZUIDAS RETROFIT · FACADE GRID', code: 'P-10', scale: 'SCALE 1:200', size: LAND },
  { file: 'proj-scheldt-lock.svg', motif: 'lock', title: 'SCHELDT LOCK · SECTION', code: 'P-11', scale: 'SCALE 1:400', size: LAND },
  { file: 'proj-noord-campus.svg', motif: 'plan', title: 'NOORD CAMPUS · SITE PLAN', code: 'P-12', scale: 'SCALE 1:1250', size: LAND },
];
...
for (const sheet of sheets) {
  ...
  const body = motifs[sheet.motif](w, h, sheet.file);
  const out = svg({ w, h, title: sheet.title, code: sheet.code, scale: sheet.scale, body });
  fs.writeFileSync(path.join(OUT, sheet.file), out, 'utf8');
  written += 1;
}
