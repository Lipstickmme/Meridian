...
function defaultFrom() {
  return process.env.FORM_FROM || process.env.NOTIFY_FROM || 'Meridian Website <onboarding@resend.dev>';
}
...
async function send(opts) {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn('[meridian] notify email failed:', res.status, text);
    ...
    console.warn('[meridian] notify email error:', err.message);
...
async function sendWebhook(subject, text, data) {
  ...
  try {
    ...
    if (!res.ok) console.warn('[meridian] notify webhook failed:', res.status);
    return res.ok;
  } catch (err) {
    console.warn('[meridian] notify webhook error:', err.message);
    return false;
  }
}
